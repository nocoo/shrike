...
| Webhook | Rust Axum | Embedded HTTP server on 127.0.0.1:7015 |
...
- [x] 3.1 Add Axum webhook server on localhost:7015
...
```bash
# Trigger sync
curl -X POST http://localhost:7015/sync -H "Authorization: Bearer <token>"

# Check status
curl http://localhost:7015/status -H "Authorization: Bearer <token>"
```
